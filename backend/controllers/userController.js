...
exports.deleteUser = catchAsyncErrors(async(req,res,next)=>{
    ...
    res.status(200).json({
        success: true,
        message:"User Deleled Successfully"
    })
})
...
exports.createEventReview = catchAsyncErrors(async(req,res,next)=>{
    ...
    const isReviewed = event.reviews.find((rev)=>rev.user.toString()===req.user._id.toString())
    if(isReviewed){
        event.review.forEach((rev) =>{
    ...
    let avg= 0;
    event.ratings = event.reviews.forEach(rev=>{
        avg+=rev.rating
